package com.github.d3.data.redis.util;

import com.github.d3.data.redis.provider.RedisDynamicDataSourceHolder;
...
public class RedisHelper {
     ...
     * Redis前缀
     */
    public static final String REDIS_PREFIX = "d3:";
    ...
    private RedisTemplate<String, Object> determineTargetRedisTemplate() {
        String targetRedisTemplateName = RedisDynamicDataSourceHolder.get();
    ...
    public boolean setObject(String key, Object value) {
        SessionCallback<Object> sessionCallback = new SessionCallback<>() {
            ...
            public List<Object> execute(RedisOperations redisOperations) {
                ...
                redisOperations.opsForValue().set(String.format("%s%s", REDIS_PREFIX, key), value);
    ...
    public boolean setObject(String key, Object value, long time) {
        SessionCallback<Object> sessionCallback = new SessionCallback<>() {
            ...
            public List<Object> execute(RedisOperations redisOperations) {
                redisOperations.multi();
                redisOperations.opsForValue().set(String.format("%s%s", REDIS_PREFIX, key), value, time, TimeUnit.SECONDS);
                return redisOperations.exec();
            }
        };
        determineTargetRedisTemplate().execute(sessionCallback);
    ...
    public Object getObject(String key) {
        SessionCallback<Object> sessionCallback = new SessionCallback<>() {
            ...
            public Object execute(RedisOperations redisOperations) {
                redisOperations.multi();
                redisOperations.opsForValue().get(String.format("%s%s", REDIS_PREFIX, key));
    ...
    public List<Object> getObjectList(String pattern) {
        Set<String> keys = determineTargetRedisTemplate().keys(String.format("%s%s", REDIS_PREFIX, pattern));
        ...
        SessionCallback<List<Object>> sessionCallback = new SessionCallback<>() {
            ...
            public List<Object> execute(RedisOperations redisOperations) {
                ...
                if (!CollectionUtils.isEmpty(v)) {
                    list.addAll((List<Object>) v.getFirst());
                }
                return list;
            }
        };
    ...
    public boolean deleteObject(String key) {
        SessionCallback<Object> sessionCallback = new SessionCallback<>() {
            ...
            public List<Object> execute(RedisOperations operations) {
                operations.multi();
                operations.delete(String.format("%s%s", REDIS_PREFIX, key));
